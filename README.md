...
FinPay microservice — ledger-service. Part of the [finpay-lab](https://github.com/finpay-lab) multi-repository
...
- Core double-entry ledger (Account + Entry), posting use-case with Outbox + idempotency.
- **AI-1 (FP-58):** `POST /ai/transactions/explain` — LLM plain-language transaction explainer.
- **AI-3 (FP-60):** `AnomalyEventConsumer` emits the `finpay_anomaly_score` Prometheus gauge (statistical + optional LLM scorer).
...
Images are built, `kind load`ed as `finpaylab/ledger-service:fp9`, and rolled out via the
`finpay-services` Argo CD ApplicationSet. The API entrypoint is the **gateway**
(port 8080); this service is reached internally on port 8081.
